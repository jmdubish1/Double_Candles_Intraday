...
def smooth_vol_oi_intra(df, close_df, securities):
    ...
    for date, group in daily_groups:
        ...
        for sec in securities:
            for metric in ['Vol', 'OpenInt', 'Close']:
    ...
    # Standard for all dataset
    for sec in securities:
        for metric in ['Vol', 'OpenInt']:
            metavg = df[f'{sec}_{metric}'] / (df[f'{sec}_{metric}'].rolling(window=21, min_periods=4).mean()) * 100
            df[f'{sec}_{metric}_Over_Avg'] = metavg.fillna(0)
